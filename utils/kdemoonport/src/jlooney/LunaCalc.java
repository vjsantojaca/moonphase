...
'c'  Date and time formatted as "%ta %tb %td %tT %tZ %tY", e.g. "Sun Jul 20 16:17:00 EDT 1969".  
	...
	// private static final String DATE_FORMAT = "%1$ta, %1$td-%1$tm-%1$tb-%1$tY %1$tH:%1$tM:%1$tS";
	...
	public static Map<Calendar, String> calcStatus(Calendar time) {
		...
		Calendar first_quarter = Phases.JDtoDate(Phases.moonphasebylunation(lun, 1), null);
		Calendar full_moon = Phases.JDtoDate(Phases.moonphasebylunation(lun, 2), null);
		Calendar third_quarter = Phases.JDtoDate(Phases.moonphasebylunation(lun, 3), null);

		int m_counter = LunaCalc.diffDays(now, last_new);
		
		int daysToLastNewMoon = LunaCalc.diffDays(now, last_new);
...
//		int daysToThirdQuarter = LunaCalc.diffDays(now, third_quarter);
		...
		phases.put(last_new, "Last new moon was " + daysToLastNewMoon + " days ago");
//		if(daysToFirstQuarter == 0)
//			phases.put(first_quarter, "First quarter is today");
//		else
//			phases.put(first_quarter, "First quarter will be in " + daysToFirstQuarter  + " days");
			
		if(daysToFullMoon == 0)
			phases.put(full_moon, "Full moon is today");
		else
			phases.put(full_moon, "Next full moon will be in " + daysToFullMoon  + " days");
//		phases.put(third_quarter, "Third quarter will be in " + daysToThirdQuarter  + " days");
		phases.put(next_new, "Next new moon will be in " + daysToNextNew  + " days");
		...
		case 5:
		case 6:
			comment.append(String.format("Waxing Crescent (New Moon was yesterday)", "Waxing Crescent (%1 days since New Moon)", m_counter));
			break;
		...
		case 12:
		case 13:
			comment.append(String.format("Waxing Gibbous (Tomorrow is Full Moon)", "Waxing Gibbous (%1 days to Full Moon)", LunaCalc.diffDays(full_moon, now)));// -fm.daysTo(
																																								// now
																																								// )
																																								// );
			break;
		...
		case 19:
		case 20:
			comment.append(String.format("Waning Gibbous (Yesterday was Full Moon)", "Waning Gibbous (%1 days since Full Moon)", LunaCalc.diffDays(full_moon,
					now)));// fm.daysTo( now ) ) );
			break;
		...
		case 27:
		case 28:
			comment.append(String.format("Waning Crescent (Tomorrow is New Moon)", "Waning Crescent (%1 days to New Moon)", LunaCalc.diffDays(next_new, now)));// -nn.daysTo(
																																								// now
																																								// )
																																								// )
																																								// );
			break;
		default:
			comment.append("too difficult to count");
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(LunaCalc.padRight("Today is", LunaCalc.TEXT_WIDTH, '.') + String.format(LunaCalc.DATE_FORMAT, now.getTime()));
		...
		while (it.hasNext()){
			...
			String desc = LunaCalc.padRight(pair.getValue(), LunaCalc.TEXT_WIDTH, '.');
			if (desc.startsWith("Current")){
				comment = desc;
			}
			else{
				System.out.println(desc + String.format(LunaCalc.DATE_FORMAT, pair.getKey().getTime()));
			}
		}
	 ...
	 * @return
	 * TODO: make this method a bit smarter.
	 */
